/**
 * SQLite database access layer for the Italian Financial Regulation MCP server.
 *
 * Schema mirrors the structure of Italian financial regulatory sources:
 *   - sourcebooks        — top-level regulatory sourcebook directories (CONSOB Regolamenti, BdI Circolari, etc.)
 *   - provisions         — individual articles, disposizioni, and guidance
 *   - enforcement_actions — CONSOB sanzioni and enforcement decisions
 *
...
export function checkProvisionCurrency(reference: string): {
  ...
  if (!row) {
    return { reference, status: "sconosciuto", effective_date: null, found: false };
  }
